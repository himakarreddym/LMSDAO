...
//	}
	...
	public List<BookCopies> readBookCopies(BookCopies bookcopies) throws SQLException {
		if(bookcopies.getBookId() !=0 && bookcopies.getBranchId()!=0 ){
			return readAll("SELECT * FROM tbl_book_copies WHERE bookId = ? AND branchId = ?", new Object[]{bookcopies.getBookId(),bookcopies.getBranchId()});
		}
		else if(bookcopies.getBookId()!=0 && bookcopies.getBranchId() ==0) {
			return readAll("SELECT * FROM tbl_book_copies WHERE bookId = ? ", new Object[]{bookcopies.getBookId()});
		}
		else if(bookcopies.getBookId() ==0 && bookcopies.getBranchId() !=0) {
			return readAll("SELECT * FROM tbl_book_copies WHERE branchId = ? ", new Object[]{bookcopies.getBranchId()});
		}
		else
		{
	...
	public List<BookCopies> readBookCopiesbyCond(BookCopies bookcopies) throws SQLException {
		return readAll("SELECT * FROM tbl_book_copies WHERE branchId = ? AND noOfCopies > 0 ", new Object[]{bookcopies.getBranchId()});
	}
	
	public BookCopies readBookCopiesByPK(Integer bookId, Integer branchId) throws SQLException {
		List<BookCopies> bookCopies = readAll("SELECT * FROM tbl_book_copies WHERE bookId = ? AND branchId = ?", new Object[]{bookId,branchId});
		if(bookCopies!=null){
