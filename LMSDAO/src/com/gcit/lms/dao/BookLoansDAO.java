...
//	}
	...
	public void updateBookLoans(BookLoans bookloans) throws SQLException {
		save("UPDATE tbl_book_loans SET dateOut = ?,dueDate=?,dateIn = ? WHERE bookId = ? AND branchId = ? AND cardNo= ? And dateOut =? ",
			new Object[] { bookloans.getDateOut(),bookloans.getDueDate(),bookloans.getDateIn() ,bookloans.getBookId(),bookloans.getBranchId(),bookloans.getCardNo(),bookloans.getDateOut() });
	}
