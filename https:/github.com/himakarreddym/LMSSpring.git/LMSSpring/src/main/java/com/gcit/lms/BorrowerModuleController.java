...
public ModelAndView gottoLibrarian(Locale locale, Model model,@RequestParam(value ="cardNo",required=false) Integer cardNo,
	...
	return new ModelAndView("borrowermenu", "Branch", Branch);
}
@RequestMapping(value = "/checkbook", method = RequestMethod.GET)
public ModelAndView gottocheckbook(Locale locale, Model model,@RequestParam(value ="cardNo",required=false) Integer cardNo,
		@RequestParam(value ="branchId",required=false) Integer branchId) {
	BookLoans bookloans = new BookLoans();
	bookloans.setBranchId(branchId);
	bookloans.setCardNo(cardNo);
...
public String editLibraryBranch1(@ModelAttribute("BookLoans")BookLoans bookloans,BindingResult result, ModelMap model) throws SQLException {
